...
public class DualWield {
	...
	private boolean canDualWield(Item item) {
		return item instanceof ItemSword || item instanceof ItemTool || item instanceof ItemHoe || item instanceof ItemTrident;
	}
	...
	private void setHand(EnumHandSide hand) {
		...
		if(player.getPrimaryHand() != hand) {
			...
			try {
				...
				FirstPersonRenderer firstPersonRenderer = mc.entityRenderer.itemRenderer;
